...
module.exports = function(proto) {
  ...
  proto._getEnvironment = function() {
    ...
    if (this.options.proxy) {
      this.logger.warn("Setting proxy: " + this.options.proxy)
      env.http_proxy = this.options.proxy;
    } else {
      this.logger.warn("No proxy set.")
    }
    return env;
  };
  ...
  proto._prepare = function(callback, readMetadata) {
    ...
    if (!readMetadata && !self.options.ffprobeData) {
